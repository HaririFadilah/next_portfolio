...
export async function generateMetadata() {
  return Meta.generate({
    ...
    baseURL: baseURL,
    image: "/images/gallery/walpapper.png",
    path: about.path,
  });
}
  ...
  return (
    <Column maxWidth="m">
      <Schema
        ...
        description={about.description}
        path={about.path}
        image='/images/galerry/walpapper.png'
        author={{
          name: person.name,
          ...
          {about.work.experiences.map((experience, index) => (
            <Column key={`${experience.company}-${index}`} fillWidth>
              ...
              {experience.roles.map((role, rIndex) => (
                <Column
                  key={`${experience.company}-${role.title}-${rIndex}`}
                  fillWidth
                  marginBottom="m"
                >
                  <Text
                  ...
                  <Column as="ul" gap="16" paddingLeft="m">
                    {role.achievements.map(
                      (achievement: JSX.Element, aIndex: number) => (
                        <Text
                          as="li"
                          variant="body-default-m"
                          key={`ach-${rIndex}-${aIndex}`}
                        >
                          {achievement}
                        </Text>
                      )
